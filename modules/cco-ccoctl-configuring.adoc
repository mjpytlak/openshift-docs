...
ifdef::nutanix[]
The Cloud Credential Operator (CCO) manages cloud provider credentials as Kubernetes custom resource definitions (CRDs). Installing a cluster on Nutanix requires that you set the CCO to `manual` mode as part of the installation process.
endif::nutanix[]
