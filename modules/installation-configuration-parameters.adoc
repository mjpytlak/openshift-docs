...
|The UUID of the subnet that is linked to the Prism Element, which is managing the {product-title} cluster. [^2^]
|String
|====
[.small]
--
1. `prismElements` lists a Prism Element (cluster) and all of the Nutanix resources that are used to host the {product-title} cluster. Only a single Prism Element is supported.
